...
<?php
	...
	while($row = pg_fetch_assoc($result)) {
		if($row['expirationtime']) {
			...
			$expires = 'Unlimited';
		$message[$counter] = "<div class='messagebox'> <img class='save' src='http://icons.iconarchive.com/icons/cornmanthe3rd/plex/512/Communication-gmail-icon.png' width='100' height='100'><p id='sender'>".$row['creatorusername']."</p><p id='posttime'>".$row['posttime']."</p><div id='expires'>".$expires."</div><p id='expirationtime' style='display:none'>".$row['expirationtime']."</p><div class ='dialog' style='display:none'>".$row['body']."</div></div>";
		$counter++;
	}
